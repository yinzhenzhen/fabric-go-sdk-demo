...
func getKeysRecursively(base string, getKey viperGetter, nodeKeys map[string]interface{}, oType reflect.Type) map[string]interface{} {
	...
	for key := range nodeKeys {
		...
		if m, ok := val.(map[interface{}]interface{}); ok {
			logger.Printf("Found map[interface{}]interface{} value for %s", fqKey)
			...
			logger.Printf("Found map[string]interface{} value for %s", fqKey)
			result[key] = getKeysRecursively(fqKey+".", getKey, m, subTypes[key])
		} else if m, ok := unmarshalJSON(val); ok {
			logger.Printf("Found real value for %s setting to map[string]string %v", fqKey, m)
			result[key] = m
		} else {
			if val == nil {
			...
			logger.Printf("Found real value for %s setting to %T %v", fqKey, val, val)
			result[key] = val
...
func unmarshalJSON(val interface{}) (map[string]string, bool) {
	...
	s, ok := val.(string)
	if !ok {
		logger.Printf("Unmarshal JSON: value is not a string: %v", val)
		return nil, false
	}
	err := json.Unmarshal([]byte(s), &mp)
	if err != nil {
		logger.Printf("Unmarshal JSON: value cannot be unmarshalled: %s", err)
		return nil, false
	}
	return mp, true
}
